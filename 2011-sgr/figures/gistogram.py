...
if (comparisonFunction != 'halo'):
	...
	for line in model:
		...
		if (len(obsPositionLimits) == 1):
			...
			if ((xPosLimits[1] > float(line[xModelIdx]) > xPosLimits[0]) and (yPosLimits[1] > float(line[yModelIdx]) > yPosLimits[0])): # wtf?
				comparVars.append(float(line[modelIdx]))
...
if (comparisonFunction != 'halo'):
	print len(histoVars), len(comparVars)
